...
| `ua-wire-check.ts` | Manual, credentialed diagnostic (#319/#345): imports the real `src/handlers/shared/ua.ts` helper and prints the assembled outbound `User-Agent` on real SDK v3 calls, proving both the SDK-native `app/` and helper-supplied `md/` segments reach the wire (CloudTrail is unavailable — DynamoDB data events are blocked) | `npx tsx scripts/ua-wire-check.ts` — see `docs/verification/319-ua-wire-runbook.md` |
