...
- [Webhook integration](/sample-autonomous-cloud-coding-agents/using/webhook-integration) credentials (HMAC secret from stack).
...
Follow [Webhook integration](/sample-autonomous-cloud-coding-agents/using/webhook-integration) to send `pull_request` events to your API Gateway URL with a valid signature.
...
The orchestrator admits the webhook, resolves `coding/pr-review-v1`, and runs the agent without write access to the default branch. See [Blueprint vs workflow](/sample-autonomous-cloud-coding-agents/architecture/blueprint-vs-workflow).

## If something fails

| Symptom | Fix |
|---------|-----|
| Webhook 401 | [Troubleshooting — webhook signatures](/sample-autonomous-cloud-coding-agents/troubleshooting/troubleshooting#webhook-signature-failures) |
| No task created | Verify event type and repo match blueprint |
| Empty review | Check agent logs via operator dashboard |

## Next steps

- [Cedar policies](/sample-autonomous-cloud-coding-agents/customizing/cedar-policies) — gate risky tools during review runs
- [Implement from issue](/sample-autonomous-cloud-coding-agents/architecture/implement-from-issue) — coding workflow pairing
