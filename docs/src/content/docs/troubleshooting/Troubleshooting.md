...
| Token works locally but not in CI | Machine principal not set up | Use API key or OIDC per [Authentication](/sample-autonomous-cloud-coding-agents/using/authentication) |
...
| Symptom | Likely cause | Fix |
|---------|--------------|-----|
| `401` on webhook endpoint | HMAC mismatch | Use secret from stack; sign raw body per [Webhook integration](/sample-autonomous-cloud-coding-agents/using/webhook-integration) |
...
| `FAILED` shortly after start | Pre-flight or guardrail block | Read `failureReason` on task; see [Task lifecycle](/sample-autonomous-cloud-coding-agents/using/task-lifecycle) |
| `AWAITING_APPROVAL` timeout | Cedar soft-deny gate | `bgagent pending` / `approve` — see [Approval gates](/sample-autonomous-cloud-coding-agents/using/overview#approval-gates-cedar-hitl) |

## Still stuck?

- [Quick Start](./QUICK_START.mdx) — verify deploy and first task
- [Learning path](/sample-autonomous-cloud-coding-agents/getting-started/learning-path) — goal-based navigation
