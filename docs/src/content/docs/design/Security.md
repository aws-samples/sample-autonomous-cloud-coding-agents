...
- **Credentials for the agent** — GitHub access supports two paths: (1) a shared or per-repo PAT stored in Secrets Manager (simplest setup), or (2) a **GitHub App** integrated via **AgentCore Identity Token Vault** (preferred for production, implemented in Iteration 3c). In the PAT path, the orchestrator reads the secret at hydration time and passes it to the agent runtime. In the Token Vault path, a `CfnWorkloadIdentity` resource represents the agent's identity and a `CfnOAuth2CredentialProvider` (vendor `GithubOauth2`) is registered with the GitHub App's credentials. At runtime, the orchestrator and agent call `GetWorkloadAccessToken` followed by `GetResourceOauth2Token` (M2M flow) to obtain a short-lived GitHub access token — the Token Vault handles the OAuth2 exchange and refresh automatically. The runtime execution role is granted `bedrock-agentcore:GetWorkloadAccessToken` (auto-granted by the AgentCore Runtime L2 construct) and `bedrock-agentcore:GetResourceOauth2Token` (explicitly granted by the `AgentCoreIdentity` construct). When Token Vault env vars (`WORKLOAD_IDENTITY_NAME`, `GITHUB_OAUTH2_PROVIDER_NAME`) are present, they are preferred over the Secrets Manager PAT. Per-repo overrides via Blueprint `credentials` support both paths. See [DEVELOPER_GUIDE.md step 4](/developer-guide/introduction) for setup and [ROADMAP.md Iteration 3c](/roadmap/roadmap) for design details.
- **Dynamic secret substitution** — the principle that **the LLM and agent context never see raw credentials**. Secrets (e.g. API keys, OAuth tokens) are held by the runtime or gateway and injected only at tool-execution time when a request is made. They do not appear in prompts, conversation history, or logs, which limits exposure from prompt leakage, log ingestion, or context exfiltration. The GitHub token (whether a PAT from Secrets Manager or an OAuth token from Token Vault) is fetched by the agent at runtime and used for git operations and GitHub API calls; the model does not receive the token in its context. With Token Vault (Iteration 3c, implemented), the agent calls `GetWorkloadAccessToken` + `GetResourceOauth2Token` to obtain a scoped, short-lived GitHub OAuth token at runtime. The GitHub App client secret is stored in Secrets Manager and accessed only by the Token Vault credential provider (never by the agent or model). Future Gateway integration will enable credential injection for GitHub API calls without any token in the sandbox.
...
- **Tool access and tiered tool profiles** — the agent's tools are allowlisted (e.g. GitHub, web search, shell, file system within the sandbox). Unrestricted tool access would increase the risk of confused deputy or unintended data exfiltration; the platform exposes only the tools needed for the task. A constrained tool surface keeps behavior more predictable and easier to evaluate. ABCA follows a **tiered tool access model**:
  ...
  - **Per-repo tool profiles:** Stored in the onboarding config and loaded by the orchestrator during context hydration. The agent harness configures the tool set based on the profile. See [REPO_ONBOARDING.md](/design/repo-onboarding) for per-repo configuration.
...
The blueprint framework (see [REPO_ONBOARDING.md](/design/repo-onboarding)) allows per-repo custom Lambda steps that execute within the orchestrator pipeline. These are a trust boundary that requires security analysis.
...
The framework passes a **filtered** `StepInput` to custom Lambda steps. The filtering policy (see [REPO_ONBOARDING.md](/design/repo-onboarding#step-inputoutput-contract)) strips credential ARNs (`github_token_secret_arn`) and networking configuration (`egress_allowlist`) from the `blueprintConfig`. Custom steps receive:
...
See [ROADMAP.md Iteration 5](/roadmap/roadmap) (Guardrails, Mid-execution behavioral monitoring).
...
The defense architecture requires six layers (see [MEMORY.md](/design/memory#defense-architecture) for the full model):
...
The review feedback memory loop (see [MEMORY.md](/design/memory)) is the most novel memory component — and the most dangerous from a security perspective. PR review comments are **attacker-controlled input** that gets processed by an LLM and stored as persistent memory influencing future agent behavior.
...
- **Single GitHub OAuth token (mitigated: GitHub App + AgentCore Token Vault, Iteration 3c)** — in the PAT path, one token may be shared for all users and repos the platform can access. Any authenticated user can trigger agent work against any repo that token can access. There is no per-user repo scoping. **Mitigation (Iteration 3c, implemented):** Deploy with a GitHub App integrated via AgentCore Token Vault. Each task obtains a short-lived OAuth token via the M2M flow. Per-repo Token Vault overrides via Blueprint `credentials` allow different GitHub Apps per repository. Combined with SSO (federated identity), tokens can be further scoped to the user's effective GitHub permissions. The PAT path remains as a simpler alternative for development. See [DEVELOPER_GUIDE.md step 4](/developer-guide/introduction) for setup and [ROADMAP.md Iteration 3c](/roadmap/roadmap) for the design.
...
- **DNS Firewall IP bypass** — DNS Firewall does not block direct IP connections (see [NETWORK_ARCHITECTURE.md](/design/network-architecture#dns-firewall)).
