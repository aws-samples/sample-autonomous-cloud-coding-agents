...
1. **Tool-call interceptor (Guardian pattern)** — A policy-evaluation layer in the agent harness (`entrypoint.py`) that sits between the agent SDK's tool-call decision and actual tool execution. Each tool call is evaluated against a policy before execution: file path restrictions (e.g. deny writes to `.github/workflows/`), bash command allowlist per capability tier, cost threshold checks, and per-repo rules from Blueprint `security` configuration. The interceptor can allow, modify (e.g. redact secrets from output), or deny tool calls. Denied calls return a structured error to the agent, which can retry with a different approach. This follows the Guardian interceptor pattern (Hu et al. 2025) — enforcement happens at tool-call time, not before the session starts (input guardrails) or after it ends (validation pipeline). Combined with per-tool-call structured telemetry (Iteration 3d), every interceptor decision is logged as a `PolicyDecisionEvent`.
