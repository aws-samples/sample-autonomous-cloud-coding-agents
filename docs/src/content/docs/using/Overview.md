...
5. **Linear** - Apply a label to a Linear issue to trigger a task; the agent posts progress comments back on the issue via Linear's MCP server. See the [Linear setup guide](/sample-autonomous-cloud-coding-agents/using/linear-setup-guide).
6. **Jira** - Add a label to a Jira Cloud issue to trigger a task; a dedicated Forge `bgagent` app posts comments and transitions through Jira REST v3 while the triggering human remains the task owner. See the [Jira setup guide](/sample-autonomous-cloud-coding-agents/using/jira-setup-guide).
