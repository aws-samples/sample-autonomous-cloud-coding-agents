...
Fine-grained tokens only work for repos you own (or orgs that have opted in). If you're a collaborator on someone else's repo, create a **classic PAT** with `repo` + `read:org` scopes instead. See [agent/README.md](/architecture/readme#github-pat--minimal-permissions) for details.
...
See [User guide → Using the REST API](/using/using-the-rest-api) for list, cancel, and events endpoints.
