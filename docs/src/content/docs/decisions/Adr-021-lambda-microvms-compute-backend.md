...
- Service error wording can hide transport errors. The pooled-hook mitigation has local and live evidence; historical internal dispatch traces remain unavailable. Open questions are tracked in the [service feedback record](/sample-autonomous-cloud-coding-agents/architecture/645-lambda-microvm-service-feedback).
