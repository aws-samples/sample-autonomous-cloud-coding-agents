...
The [payload contract](/sample-autonomous-cloud-coding-agents/verification/645-payload-bootstrap) and [live checks](/sample-autonomous-cloud-coding-agents/verification/readme) record the implementation and validation. This transport does not establish complete hostile-worker isolation: other platform grants remain, and a stolen signed URL is usable until expiry or revocation.
...
The [verification summary](/sample-autonomous-cloud-coding-agents/verification/readme) distinguishes recorded live acceptance from open PR checks. Required coverage includes:
...
- [ADR-020](/sample-autonomous-cloud-coding-agents/decisions/adr-020-ears-requirements-syntax) — requirement syntax
