...
- (−) **A new schema and validator to maintain** (JSON Schema + synth-time/CI lint over `agent/workflows/**`), plus the `workflow_ref`/`resolved_workflow` fields now span the CDK↔CLI type-sync contract.
...
- (!) **Repo-optionality is a wider refactor than Phase 3's one-liner implies** — `repo` is required across ~6 TS interfaces + the agent config validator, and memory keying, SessionRole tenant tags, and artifact delivery all assume a repo. Tracked as blocking open questions.
