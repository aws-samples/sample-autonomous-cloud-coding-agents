...
| `approval_gate_count` | N | No | Running counter of approval gates fired on this task; used to enforce `approvalGateCap` (decision #13) |
...
Emitted to both `ProgressWriter` (DDB, 90d) and `sse_adapter` (live stream). Plus audit events emitted by the REST Lambdas directly to TaskEventsTable. Naming is consistent: `approval_requested` → `approval_granted` / `approval_denied` / `approval_timed_out` / `approval_stranded` (never `approval_decided` — use the specific outcome). See finding #13.
...
| `approval_cap_exceeded` | Agent | `{request_id, count, cap}` — when `approvalGateCap` fires (cap field reflects the configured value) |
...
| 46 | cdk | `test/constructs/task-dashboard.test.ts` | Dashboard wiring tests (IMPL-28): assert `ApprovalTimeoutClipRate`, `ApprovalTimeoutBreakdown`, and `ApprovalDecisionLatency` widgets exist with correct dimensions; old bundled "Approval response time" widget is retired. |
...
- [ ] **Timeout-tuning metrics (IMPL-28)**: `ApprovalTimeoutClipRate` (dim: reason), `ApprovalTimeoutBreakdown` (histogram, dim: rule_id), `ApprovalDecisionLatency` (p50/p90/p99, dim: outcome) wired into `TaskDashboard`. Old bundled "Approval response time" widget retired. See §11.3.
