...
> command path) · `5c31b1c` T2 (warm digest) · `5fb96c5` the PR link on the ✅ completion comment ·
> `6c1f368` a `:decompose` that declines to split goes behind the same approval gate · `ff37340`
> semantic revise matures the ONE plan comment in place + settles the feedback comment 👀→✅; no
> split-thread reply. All verified end-to-end on a live workspace
> (AgentCore) except the PR-link fix (verified in the same run as the single-task gate) — see
> per-thread STATUS blocks + the findings below. Remaining: T6/T7 (measure-first, deferred); ECS right-sized
> planning (separate workstream, `ECS_RIGHTSIZED_PLANNING.md`).
>
> **Status:** PROPOSAL for review by the orchestration-reporting fix session before any code lands.
> **Author:** QA/design pass (session 958d5e85), 2026-07-06.
> **Branch discipline:** HOLD — do not start until the orchestration-reporting fix session finishes
...
Auto-decomposition planning is correct but **feels slow and clunky**, and it isn't the
...
- **No held live session by default.** Approval is on human-time (minutes→days). Holding a
  ...
  complexity — exactly what the auto-decomposition design avoids (plan checkpoints to S3 + a pending-plan DDB row with a
...
### T1 — Close the short-negation-with-instruction gap (TACTICAL, ship first) ✅ SHIPPED + VERIFIED
> **STATUS: DONE.** Implemented in a separate worktree, commit `c45c8bf` on branch
> `fix/492-t1-short-negation` (branched from `be933e9`), deployed to dev and verified live,
...
> mention. Live matrix: `no, just 2 tasks` → **revise** (plan survived, "Updated
...
revise, not discard — verified live), and a bare `@bgagent` (empty instruction) → nudge
(also verified live). **Do NOT rebuild that.** T1's remaining target is the ONE residual I caught
live:

- **The residual (caught in live use):** `@bgagent no, just 2 tasks` / `no, make it 3 tasks`
...
### T2 — Warm repo digest, cache the exploration across revise rounds ✅ SHIPPED + VERIFIED
> **STATUS: DONE + committed `5c31b1c`** on `fix/492-t1-short-negation` (stacked on T5). Full monorepo
> build green (agent 1197 + cli 575 + cdk 2929 tests + synth + docs). Deployed to dev and **verified
> live**: round-0 stored `repo_digest_sha b54d4786…` + a 1080-char structural digest; a SEMANTIC
...
> bug found in live use and fixed en route: `repo.py` only captured `head_sha_before` on the
> PR-workflow branch, so the decompose task's sha was empty — now captured for non-PR clones too. NOT pushed.
>
...
> re-deriving it. **Honors the no-platform-GitHub-token constraint**: only the agent knows the sha (it clones),
...
> pre-check without a platform-side GitHub token.
...
- **Correctness backstops (non-negotiable — this is what keeps it from regressing to the blind
  planner that caused the decompose timeout + repo-context regressions):**
...
### T4 — Direct-manipulation command grammar (BIGGEST "instant" win) ✅ SHIPPED + VERIFIED
...
> Full CDK build green (2916 tests, +14). Deployed to dev and **verified live** on a 6-node plan:
...
- **Reuse existing idempotency/claim-once guards** (the webhook-redelivery spam bug) — editing one comment
...
- **Developer at a terminal:** don't make server-side planning compete with a warm local Claude — it
  can't win. Make **"plan locally → create sub-issues → label parent → ABCA runs the declared graph
...
- Idempotency (T5): GitHub ("respond 2XX within 10s or the delivery is a failure") + Stripe ("endpoints
  ...
  claim-once / dedup approach already in the codebase. Ack fast, offload work.
