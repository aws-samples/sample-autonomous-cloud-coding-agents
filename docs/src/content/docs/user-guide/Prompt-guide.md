...
This guide covers how to write effective task descriptions, common anti-patterns to avoid, and tips for getting the most out of the platform. For submission mechanics (CLI flags, API fields, webhook setup), see the [User guide](/user-guide/introduction).
...
For `pr_iteration` tasks (when using `--pr`), the user prompt has a different structure:
...
- **Pull Request context** (title, body, diff, review comments) — included when you use `--pr` (`pr_iteration`).
...
You must provide at least one of `--issue`, `--task`, or `--pr`.
