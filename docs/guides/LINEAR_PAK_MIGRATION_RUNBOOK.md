...
> **Who needs this.** Operators who deployed Phase 2.0a (single Linear personal API key shared across all teammates) and need to upgrade to 2.0b (per-workspace OAuth). If you're starting fresh on 2.0b, read [LINEAR_SETUP_GUIDE.md](./LINEAR_SETUP_GUIDE.md) instead.
...
3. **For each Linear workspace**, follow the [setup walkthrough](./LINEAR_SETUP_GUIDE.md#setup-walkthrough) starting at step 2. Each workspace needs:
