...
| `self_review` | agentic | Run a read-only critic loop over the cumulative branch diff and post a structured findings comment on the PR | Advisory and fail-open (`on_failure: continue`); never gates the verdict. Repo-only (requires a prior `clone_repo` + `run_agent`). Non-side-effecting: the critic is forced `read_only` (no `Write`/`Edit`). Turn budget drawn from the task's remaining allowance, capped by `max_turns` (default `5`); an optional `prompt` overrides the built-in review template. |
