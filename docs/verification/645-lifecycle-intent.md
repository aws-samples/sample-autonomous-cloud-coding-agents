...
A caller must supply valid times, session expiry, its normal poll interval, an enable switch and an explicit compatible-image flag. Both flags are required for a new suspend. Capability must describe the image/version that launched **this VM**, backed by coordinator-owned launch metadata or a verified full drain; current deployment settings alone cannot authenticate an older VM's capability. Unknown/legacy capability keeps new suspends off. Disabling new suspends still permits wake and termination. Long poll intervals are shortened to the next grace/wake/session deadline.
