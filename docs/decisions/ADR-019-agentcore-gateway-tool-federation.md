...
Linear and Jira MCP are **explicitly not in near-term scope.** Linear is deterministic by decision ([ADR-016](./ADR-016-pluggable-identity-and-auth.md)) and stays that way — this ADR does not re-introduce an agent-side Linear MCP. Jira's live path is the REST shim ([ADR-015](./ADR-015-jira-integration.md)); whether a gateway that owns the OAuth flow could *unbreak* the non-functional Jira MCP placeholder is recorded below as a **speculative later experiment**, explicitly not part of the deterministic reaction/orchestration paths.

> **Not to be confused with the input gateway.** [`INPUT_GATEWAY.md`](../design/INPUT_GATEWAY.md) describes ABCA's *inbound channel normalization* — turning CLI / Slack / webhook payloads into one internal message format on the way **in**. AgentCore Gateway here is the opposite direction: aggregating *outbound tool calls* the agent makes **out**. They share the word "gateway" and nothing else.
...
- **Inbound principal verification and outbound credential resolution.** Owned by [ADR-016](./ADR-016-pluggable-identity-and-auth.md); this ADR is the tool-plane consumer of both seams.
- **The Linear/Jira deterministic reaction paths.** `linear_reactions.py` / `jira_reactions.py` stay as-is and authoritative. This ADR does **not** re-introduce an agent-side Linear or Jira MCP: Linear stays deterministic ([ADR-016](./ADR-016-pluggable-identity-and-auth.md), enforced by `strip_linear_mcp_servers()`), and Jira's live path stays the REST shim ([ADR-015](./ADR-015-jira-integration.md)). The speculative Jira-MCP-via-gateway experiment (P4) is gated separately and touches neither path.
...
- [ADR-016](./ADR-016-pluggable-identity-and-auth.md) — pluggable identity and auth (the credential-plane seams this ADR consumes)
- [ADR-015](./ADR-015-jira-integration.md) — Jira integration (the REST-shim precedent for a non-functional MCP placeholder)
- `docs/design/AGENTCORE_GATEWAY_MCP_SPIKE.md` — the GO spike, findings F0–F16, and the provisioning recipe + gotchas. Lives only on the reference branch `feat/agentcore-gateway-mcp` (upstream remote); not merged to `main`.
- [`INPUT_GATEWAY.md`](../design/INPUT_GATEWAY.md) — the *inbound channel* gateway, distinct from AgentCore Gateway
