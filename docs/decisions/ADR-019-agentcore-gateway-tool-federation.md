...
- **Jira / Atlassian Remote MCP** (`https://mcp.atlassian.com/v1/sse`) — a **non-functional placeholder** today (headless OAuth 2.1 cannot connect, so the live path is the REST shim in `jira_reactions.py`, [ADR-015](./ADR-015-jira-integration.md)). Fronting it with a gateway that owns the OAuth flow is the case most likely to *unbreak* MCP for Jira.
