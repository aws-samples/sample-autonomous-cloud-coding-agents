...
class TestConftestScrubsScopingEnv:
    """Regression guard for the ECS test-hang.
    ...
    silently. The ``_clean_env`` autouse fixture in conftest MUST strip the var
    (resetting the session cache alone is not enough — a cold ``get_session()``
    re-resolves scoped while the var is still set).
    """
